# Summary

* [Agents.JSON Overview](README.md)
* [Agents.JSON Specification](agents-json.md)

## Guides

* [Documentation Index](docs/index.md)
* [Implementer’s Guide](docs/implementers-guide.md)
* [Framework & Format Mapping](docs/mapping-frameworks.md)
* [Extensions Guide](docs/extensions.md)

## Schemas

* [Core Manifest Schema](schema/agents-json.json)
* [Message Envelope](schema/message-envelope.json)
* Capabilities
  * [Summarization](schema/capabilities/summarization.json)
  * [Routing](schema/capabilities/routing.json)
  * [Retrieval](schema/capabilities/retrieval.json)
* Extensions
  * [Audit Extension](schema/extensions/audit.json)
  * [Memory Extension](schema/extensions/memory.json)

## Registry

* [Profiles](registry/profiles.json)
* [Capabilities](registry/capabilities.json)
* [Tool Types](registry/tool-types.json)
* [Extensions](registry/extensions.json)

## Examples

* [Core Manifest](examples/core.json)
* [Core + Exec](examples/core-exec.json)
* [Core + Exec + Gov](examples/core-exec-gov.json)
* [Core + Exec + Gov + Graph](examples/core-exec-gov-graph.json)

## Project

* [Contributing](CONTRIBUTING.md)
* [Changelog](CHANGELOG.md)
* [License](LICENSE)
